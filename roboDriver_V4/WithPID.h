...
/**
 * @brief Check if PID should execute based on frequency control
 * @return true if enough time has elapsed for next PID execution
 */
bool should_execute_pid(void);

/**
 * @brief Execute PID controllers at controlled frequency for smooth movement
 * @param motor_output_L Pointer to left motor output (updated if PID executes)
 * @param motor_output_R Pointer to right motor output (updated if PID executes)
 */
